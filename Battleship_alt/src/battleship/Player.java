...
public class Player
{
	...
	private boolean[][] hitormiss=new boolean[11][11];
	private boolean chit=false;//checks if computer hit ship or not		
	public  JButton[][] bboard = new JButton [12][12];
						//gbutton=new JButton [11][11];
	private int[][] mhs=new int[11][11];//used by computer to track miss(0)
	...
	private String[][] whatship=new String[11][11];//stores name of ships or " "
	...
	{			
		...
		{			
			for (j=0;j<11;j++)
			{
				...
				this.bboard[i][j].setPreferredSize(new Dimension(30, 30));
				Border emptyBorder = BorderFactory.createEmptyBorder();
				this.bboard[i][j].setBorder(emptyBorder);
				if(i%2==0 && j%2==0)	this.bboard[i][j].setBackground(Color.white);
				else if(i%2==1 && j%2==0)	this.bboard[i][j].setBackground(light_green);
				else if(i%2==0 && j%2==1)	this.bboard[i][j].setBackground(light_green);
				else if(i%2==1 && j%2==1)	this.bboard[i][j].setBackground(dark_green);			
				hitormiss[i][j]=false;
				this.whatship[i][j]=" ";				
			}
		}			
	...
	public void setBboard(int i,int j, ImageIcon k)
	{
		this.bboard[i][j].setIcon(k);	
	}
